...
bool NormalMapShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, XMMATRIX worldMatrix, XMMATRIX viewMatrix, XMMATRIX projectionMatrix,
								 ...
								 XMFLOAT3 lightDirection, XMFLOAT4 diffuseColor, XMFLOAT3 lightDirectionPhong, XMFLOAT4 diffuseColorPhong)
{
	bool result;


	// Set the shader parameters that it will use for rendering.
	result = SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, texture1, texture2, lightDirection, diffuseColor, lightDirectionPhong, diffuseColorPhong);
	if(!result)
	{
		return false;
	}
...
bool NormalMapShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	result = device->CreateBuffer(&lightBufferDesc, NULL, &m_lightBufferPhong);
	if (FAILED(result))
	{
		return false;
	}
...
void NormalMapShaderClass::ShutdownShader()
{
	...
	if (m_lightBufferPhong)
	{
		m_lightBufferPhong->Release();
		m_lightBufferPhong = 0;
	}
											  ...
											  XMFLOAT3 lightDirection, XMFLOAT4 diffuseColor, XMFLOAT3 lightDirectionPhong, XMFLOAT4 diffuseColorPhong)
{
	...
	// ----- 조명2 (Phong) ----- //
	result = deviceContext->Map(m_lightBufferPhong, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
	if (FAILED(result))
	{
		return false;
	}
	...
	dataPtr3->diffuseColor = diffuseColorPhong;
	dataPtr3->lightDirection = lightDirectionPhong;
	dataPtr3->padding = 0.0f;

	deviceContext->Unmap(m_lightBufferPhong, 0);

	bufferNumber = 1;

	deviceContext->PSSetConstantBuffers(bufferNumber, 1, &m_lightBufferPhong);
